...
## 2026-09-23 05:36 PT · iteration 23 · KEEP (G lead holds · C spike fully mean-reverted · pre-RTH)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~10.9h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=7` (unchanged); stock `errors=1` (unchanged). Max crypto latency ~35s once; avg ~0.29s.
- **Crypto post-restart (~4212 ticks / ~12.0h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$28.0 / 65 fills / 8 open**. Shadows by PnL: **G≈+$28.0/65f > D=0f/flat > A≈F≈−$18.1/67f > H≈−$35.1/61f > B≈−$39.2/42f > C≈−$57.7/21f > E≈−$631/1244f**. Regime mix all-run ~60% chop / ~40% trend (last-200 ~42% chop / 58% trend); trade_imbalance still ≈ 0. Live regime `chop`.
- **G≠A:** 32/65 primary fills opposite side vs A. **33 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A): prior 31 + 2 new this window (FARTCOIN sells t4104/4106). Chop fades remain opposite A. `fade_applied=true` on **all 65/65** G fills (metadata always-on).
- **Hourly story:** iter-22 G lead ~$43 (G≈+$26 vs A≈−$16) **holds** — G now ≈+$28 vs A≈−$18 (G lead ~$46). PnL move is mostly MTM on existing books, not new alpha. **BONK** short still the main driver: u≈+$32 on ~$1.47k notional. **FARTCOIN** short u≈+$15 on ~$0.59k (2 new trend sells this hour, same-as-A). Other G open: ZEN/VVV soft red; API3/PYTH/ZEC small green; XRP near flat. No stale/zero-mid marks.
- **C spike fully mean-reverted (do not promote):** iter-22 C ≈+$65.3/17f → now ≈**−$57.7/21f**. L3 long that dominated the spike gave back the MTM — same false-edge pattern as B’s iter-21 spike. Reinforces multi-hour + RTH persistence gate.
- **B stays red** after full mean-reversion (≈−$39/42f); no new thin-book spike contenders.
- **Last ~1h:** only **2** new G fills, both FARTCOIN trend sells same-as-A (t4104/4106). Capacity still full at 8 names. Fill concentration all-run: BONK 30 / FARTCOIN 16 / VVV 8 / ZEN 5.
- **Stock overnight (~840 ticks):** primary A flat **$0 / 0 fills**; all shadows flat. `session_ok=false` on **all** post-restart ticks; ~111 sized signals (AMZN/NOW/SCHW/XLE/…) correctly blocked (no fills). Expected pre-RTH (`last_ts` ~05:25 PT; RTH open ~06:30 PT).
- JSONL append-only: `logs/raw_decisions/crypto.jsonl` ~6270 lines; `stock.jsonl` ~1190. Out run books remain the post-17:37 re-init wave.

### Lessons learned
1. **Good / KEEP validated again:** G lead held for a second consecutive hour (~$43→~$46) without needing a CHANGE; overnight A↔G oscillation has stabilized temporarily on the fade side via BONK/FARTCOIN shorts. Still **do not CHANGE on hourly ranks** — first real test is post-RTH persistence.
2. **Good / C spike rule confirmed:** The +$65 C lead fully mean-reverted to ≈−$58 within ~1h — same as B’s false spike. Escalate rule (c) continues to require multi-hour edge surviving mean-reversion + RTH.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (33 trend same-side cases). Side selection in trend still matches A; no chop wrong-side bugs. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay ~19× G fills and deepest red (~−$631). H ≲ A ≡ F. D still 0 fills (imbalance≈0).
5. **Watch / concentration:** BONK ~$1.5k short + FARTCOIN ~$0.6k short remain the main G path-dependence — currently green and the lead’s mechanical driver. Observe through RTH open rather than restart.
6. Stock still uninformative overnight; **first stock judgment starts at next RTH open (~06:30 PT)**.
...
## 2026-09-23 07:51 PT · iteration 25 · KEEP (G lead holds through first RTH hour · stock sample building)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~8.7h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=7` (unchanged); stock `errors=1` (unchanged). Max crypto latency ~35s once; avg ~0.29s.
- **Crypto post-restart (~4974 ticks / ~14.2h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$163.7 / 69 fills / 8 open**. Shadows by PnL: **G≈+$163.7/69f > A≈F≈+$48.2/76f > H≈+$12.8/70f > D=0f/flat > B≈−$133/53f > C≈−$137/44f > E≈−$738/1437f**. Regime mix all-run ~59% chop / ~41% trend (last-200 ~37% chop / 63% trend); trade_imbalance still ≈ 0. Live regime `chop`.
- **G≠A:** 32/69 primary fills opposite side vs A (all chop fades). **37 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A): prior 36 + 1 new this window (BONK sell t4789). `fade_applied=true` on **all 69/69** G fills (metadata always-on).
- **Hourly story:** iter-24 G lead ~$87 (G≈+$65 vs A≈−$22) **holds and widens on absolute PnL** — G now ≈+$164 vs A≈+$48 (G lead ~$116). Move is mostly MTM on existing shorts, not a flood of new alpha. **BONK** short still the main driver: u≈+$138.6 on ~$1.46k notional. **FARTCOIN** short u≈+$43.3 on ~$0.86k. Other G open: API3/PYTH small green; VVV/ZEN/XRP/ZEC soft red. No stale/zero-mid marks.
- **Last ~1h:** only **1** new G fill — BONK trend sell same-as-A (t4789 ~07:17 PT). Capacity still full at 8 names. Fill concentration all-run: BONK 31 / FARTCOIN 19 / VVV 8 / ZEN 5.
- **Shadows:** B/C stayed deep red after prior false spikes (no new thin-book spike contenders). E exit overlay ~21× G fills and deepest red (~−$738). H ≪ A ≡ F. D still 0 fills (imbalance≈0). A flipped from iter-24 red to green (~−$22→+$48) via its own BONK/FARTCOIN path, but still trails G’s fade book.
- **Stock RTH (~1.2h into open):** ~924 ticks; decisions `session_ok` False×876 / True×50. Primary **A ≈ +$4.01 / 12 fills / 8 open** — all shorts post-`session_ok=true`: TSLA u≈+$2.3 (~$0.40k), ARM u≈+$1.4 (~$0.30k), GOOGL u≈+$0.8, SPY/META small green, DIS soft red, ZM/SNAP flat. Shadows: **B≈C ≈+$4.6/8f ≳ A≈F≈H ≈+$4.0/12f > G ≈+$2.4/12f > E ≈+$0.1/20f > D=0**. Sample still thin; no stock primary call yet (`last_ts` ~07:44 PT).

### Lessons learned
1. **Good / KEEP validated through first RTH hour:** G lead held a fourth consecutive hour and widened on absolute PnL (~$87→~$116 lead) without a mid-wave CHANGE. Escalate rule (a) still not met — needs *sustained G≪A after RTH with fade losing on the same names across several post-open hours*; here fade (BONK/FARTCOIN) is winning.
2. **Watch / concentration:** BONK ~$1.5k short + FARTCOIN ~$0.9k short still dominate G path-dependence (together ~+$182 unrealized vs book +$164 — other names net soft). Wide lead remains mechanical/MTM; do not CHANGE on this alone.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (37 trend same-side cases). Side selection in trend still matches A; no chop wrong-side bugs. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$738). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns (now deep red).
5. **Stock / sample building:** RTH gate still working (fills only after `session_ok=true`); A book full at 8 shorts, PnL still noise-level (~+$4). Need several more RTH hours before any stock primary keep/discard call. B/C tiny lead over A is not meaningful yet.
...
### Next observe window
- Next hourly reflect ~08:08–08:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM holds through mid-RTH, whether A’s crypto rebound narrows the lead without flipping it, and whether stock A’s short book builds a clearer edge beyond ~+$4.

## 2026-09-23 08:58 PT · iteration 26 · KEEP (G lead holds mid-RTH · lead narrows vs A rebound · stock ~flat)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~7.5h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=7` (unchanged); stock `errors=1` (unchanged). Max crypto latency ~35s once; avg ~0.29s.
- **Crypto post-restart (~5370 ticks / ~15.4h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$221.1 / 73 fills / 8 open**. Shadows by PnL: **G≈+$221.1/73f > A≈F≈+$127.7/85f > H≈+$83.4/79f > D=0f/flat > C≈−$131.8/47f > B≈−$157.8/60f > E≈−$767.4/1499f**. Regime mix all-run ~58% chop / ~42% trend (last-200 ~34% chop / 66% trend); trade_imbalance still ≈ 0. Live regime `chop`.
- **G≠A:** 33/73 primary fills opposite side vs A (all chop fades; 33/33). **40 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A): prior 37 + 3 new this window (VVV buys/sells). `fade_applied=true` on **all 73/73** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs.
- **Hourly story:** iter-25 G lead ~$116 (G≈+$164 vs A≈+$48) **holds but narrows** — G now ≈+$221 vs A≈+$128 (G lead ~$93). Absolute G PnL still rose (~+$57) while A rebounded harder (~+$80), mostly MTM. **BONK** short still the main G driver: u≈+$179.5 on ~$1.42k notional. **FARTCOIN** short u≈+$63.2 on ~$0.84k. Other G open: API3/PYTH/VVV small green; ZEN soft red (~−$32); XRP/ZEC soft red. No stale/zero-mid marks. Note A’s BONK is **long** (~flat) while G’s BONK is the winning short — fade path-dependence remains the lead’s mechanical core.
- **Last ~1h:** **4** new G fills, all **VVV-USD**: t4995 trend buy same-as-A; t5314 chop fade buy opp-A (large_down → G buy / A sell); t5352/5354 trend sells same-as-A. Capacity still full at 8 names. Fill concentration all-run: BONK 31 / FARTCOIN 19 / VVV 12 / ZEN 5.
- **Shadows:** B/C stay deep red after prior false spikes (no new thin-book spike contenders). E exit overlay ~21× G fills and deepest red (~−$767). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock RTH (~2.5h into open):** ~967 ticks; `session_ok` False×876 / True×90. Primary **A ≈ +$0.99 / 19 fills / 8 open** — mostly shorts (SPY/TSLA/DIS/ZM/GOOGL/SNAP/ARM) + META long; uPnL all noise-level (|u|≲$1.2). Shadows: **A≈F≈H ≈+$1.0/19f ≳ G ≈+$0.9/19f > C ≈+$0.5/12f > D=0 > E ≈−$0.8/44f > B ≈−$2.2/15f**. A gave back iter-25’s ~+$4 → ~+$1; still uninformative for primary keep/discard (`last_ts` ~08:54 PT).

### Lessons learned
1. **Good / KEEP validated mid-RTH:** G lead held a fifth consecutive hour (~$116→~$93) without a mid-wave CHANGE. Absolute G book still climbing; lead narrowed because A rebounded on its own shorts (esp. FARTCOIN), not because fade flipped red. Escalate rule (a) still not met — needs *sustained G≪A after RTH with fade losing on the same names across several post-open hours*.
2. **Watch / concentration:** BONK ~$1.4k short + FARTCOIN ~$0.8k short still dominate G path-dependence (together ~+$243 unrealized vs book +$221 — other names net soft, ZEN −$32). Lead remains mechanical/MTM; do not CHANGE on this alone.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (40 trend same-side cases). Side selection in trend still matches A; chop fades still 100% opposite A. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$767). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns.
5. **Stock / still noise:** RTH gate working; A book full at 8 names but PnL oscillates ~+$4→+$1. Need several more RTH hours (and ideally into close) before any stock primary call. No shadow edge surviving multi-hour + mean-reversion.
...
### Next observe window
- Next hourly reflect ~09:08–09:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM holds through late morning RTH, whether A’s rebound continues to close the ~$93 lead, and whether stock A’s short book leaves noise territory.

## 2026-09-23 09:55 PT · iteration 27 · KEEP (G lead holds late morning RTH · A rebound stalls · stock still noise)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~6.6h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=7` (unchanged); stock `errors=1` (unchanged). Max crypto latency ~35s once; avg ~0.29s.
- **Crypto post-restart (~5712 ticks / ~16.3h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$233.9 / 76 fills / 8 open**. Shadows by PnL: **G≈+$233.9/76f > A≈F ≈+$142.7/87f > H≈+$93.5/82f > D $0/0f > C≈−$143.8/50f > B≈−$231.0/68f > E≈−$799.5/1584f**. Regime mix all-run ~57% chop / ~42% trend (last-200 ~52% chop / 48% trend); trade_imbalance still ≈ 0. Live regime `chop`.
- **G≠A:** 33/76 primary fills opposite side vs A (all chop fades; 33/33). **42 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A): prior 40 + 2 new with A shadow (FARTCOIN sell t5399, PYTH buy t5531); **1 no-A** (FARTCOIN sell t5400, A did not fill). `fade_applied=true` on **all 76/76** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs.
- **Hourly story:** iter-26 G lead ~$93 (G≈+$221 vs A≈+$128) **holds** — G now ≈+$234 vs A≈+$143 (G lead ~$91). Absolute G PnL edged up (~+$13) while A’s rebound **stalled** (~+$15, much slower than last hour’s ~+$80). Still mostly MTM on existing shorts. **BONK** short still the main G driver: u≈+$187.6 on ~$1.41k notional. **FARTCOIN** short u≈+$72.8 on ~$1.03k (2 new trend sells this window). Other G open: PYTH/API3 small green; VVV near flat; ZEC/XRP soft red; ZEN soft red (~−$32). No stale/zero-mid marks. A’s BONK remains a tiny **long** (~flat) while G’s BONK short keeps the lead’s mechanical core.
- **Last ~1h:** **3** new G fills — FARTCOIN trend sells t5399/5400 (same-as-A / no-A) + PYTH trend buy t5531 same-as-A. Capacity still full at 8 names. Fill concentration all-run: BONK 31 / FARTCOIN 21 / VVV 12 / ZEN 5 / PYTH 3.
- **Shadows:** B/C stay deep red after prior false spikes (B worsened ~−$158→−$231; no new thin-book spike contenders). E exit overlay ~21× G fills and deepest red (~−$800). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock RTH (~3.5h into open):** ~1002 ticks; `session_ok` False×876 / True×125. Primary **A ≈ +$3.75 / 23 fills / 8 open** — mostly shorts (TSLA/GOOGL/ARM/SPY green; DIS soft red; ZM/SNAP flat) + META long soft red. New this hour: GOOGL sells t970/988, META buy t989 + sell t1002. Shadows: **A≈F≈H ≈+$3.8/23f ≳ C ≈+$3.5/14f > G ≈+$1.8/22f > B ≈+$1.1/17f > D=0 > E ≈−$1.4/67f**. Still noise-level; no stock primary call (`last_ts` ~09:53 PT).

### Lessons learned
1. **Good / KEEP validated late morning RTH:** G lead held a sixth consecutive hour (~$93→~$91) without a mid-wave CHANGE. A’s prior rebound stalled; fade (BONK/FARTCOIN) still winning. Escalate rule (a) still not met — needs *sustained G≪A after RTH with fade losing on the same names across several post-open hours*.
2. **Watch / concentration:** BONK ~$1.4k short + FARTCOIN ~$1.0k short still dominate G path-dependence (together ~+$260 unrealized vs book +$234 — other names net soft, ZEN −$32). Lead remains mechanical/MTM; do not CHANGE on this alone.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (42 trend same-side cases). Side selection in trend still matches A; chop fades still 100% opposite A. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$800). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns (B deepening).
5. **Stock / still noise:** RTH gate working; A book full at 8 names but PnL oscillates ~+$1→+$4→+$4. Need several more RTH hours (into afternoon / close) before any stock primary call. No shadow edge surviving multi-hour + mean-reversion.
...
### Next observe window
- Next hourly reflect ~10:08–10:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM holds through midday RTH, whether A stays stalled or resumes closing the ~$91 lead, and whether stock A’s short book leaves noise territory into the afternoon.

## 2026-09-23 10:43 PT · iteration 28 · KEEP (G lead holds midday RTH · FARTCOIN adds · stock still noise)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~5.8h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=7` (unchanged); stock `errors=1` (unchanged). Max crypto latency ~35s once; avg ~0.29s.
- **Crypto post-restart (~5988 ticks / ~17.1h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$229.3 / 78 fills / 8 open**. Shadows by PnL: **G≈+$229.3/78f > A≈F ≈+$142.0/87f > H ≈+$91.0/84f > D $0/0f > C≈−$172/53f > B≈−$232/69f > E≈−$906/1650f**. Regime mix all-run ~57% chop / ~43% trend (last-200 ~48% chop / 52% trend); trade_imbalance still ≈ 0. Live regime `trend` at reflect time.
- **G≠A:** 33/78 primary fills opposite side vs A (all chop fades; 33/33). **42 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A). **3 no-A** (incl. 2 new this window). `fade_applied=true` on **all 78/78** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs.
- **Hourly story:** iter-27 G lead ~$91 (G≈+$234 vs A≈+$143) **holds** — G now ≈+$229 vs A≈+$142 (G lead ~$87). Absolute G PnL edged down slightly (~−$5 MTM) while A was roughly flat; lead stable. **BONK** short still the main G driver: u≈+$187.6 on ~$1.41k notional. **FARTCOIN** short u≈+$73.8 on ~$1.23k (2 new trend sells this window, both no-A). Other G open: API3/PYTH small green; VVV near flat; ZEC/XRP soft red; ZEN soft red (~−$33). No stale/zero-mid marks. A’s BONK remains a tiny **long** (~flat/−$1) while G’s BONK short keeps the lead’s mechanical core; A’s FARTCOIN short is larger (~$1.45k, u≈+$149) so A’s rebound capacity is real but did not fire this hour.
- **Last ~1h:** **2** new G fills — FARTCOIN trend sells t5794/t5968 (both no-A, `large_down`, fade flag true on trend path). Capacity still full at 8 names. Fill concentration all-run: BONK 31 / FARTCOIN 23 / VVV 12 / ZEN 5 / PYTH 3.
- **Shadows:** B/C stay deep red (no new thin-book spike contenders). E exit overlay ~21× G fills and deepest red (~−$906 on `pnl` field; equity implies ~−$1.3k — exit-overlay accounting noise, still discard). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock RTH (~4.5h into open):** ~1026–1031 ticks; `session_ok` False×876 / True×154. Primary **A ≈ +$3.45 / 25 fills / 8 open** — shorts TSLA/GOOGL/ARM/SPY green; DIS/META soft red; ZM/SNAP flat. New this hour: META buy t1012 + sell t1021, TSLA buy t1030 (cover/add under `session_ok=true`). Shadows: **C ≈+$5.5/15f ≳ B ≈+$5.3/18f ≳ G ≈+$4.9/24f > A≈F≈H ≈+$3.5/25f > D=0 > E ≈−$3/79f**. Still noise-level; thin C/B edge is not multi-hour durable (`last_ts` ~10:33 PT on live snapshot).

### Lessons learned
1. **Good / KEEP validated midday RTH:** G lead held a seventh consecutive hour (~$91→~$87) without a mid-wave CHANGE. Fade (BONK/FARTCOIN) still winning; A rebound stayed stalled. Escalate rule (a) still not met — needs *sustained G≪A after RTH with fade losing on the same names across several post-open hours*.
2. **Watch / concentration:** BONK ~$1.4k short + FARTCOIN ~$1.2k short still dominate G path-dependence (together ~+$261 unrealized vs book +$229 — other names net soft, ZEN −$33). Lead remains mechanical/MTM; do not CHANGE on this alone.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (42+ trend same-side cases; 2 new trend no-A adds). Side selection in trend still matches A when A also fills; chop fades still 100% opposite A. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$906). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns on crypto (stock C/B tiny green is noise).
5. **Stock / still noise:** RTH gate working; A book full at 8 names but PnL oscillates ~+$1→+$4→+$3.5. Need into afternoon / close before any stock primary call. No shadow edge surviving multi-hour + mean-reversion.
...
### Next observe window
- Next hourly reflect ~11:08–11:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM holds through afternoon RTH, whether A’s larger FARTCOIN short starts closing the ~$87 lead, and whether stock A leaves noise territory into the afternoon.

## 2026-09-23 11:39 PT · iteration 29 · KEEP (G lead widens afternoon RTH · VVV chop fades correct · stock still noise)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~4.9h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=7` (unchanged); stock `errors=1` (unchanged). Max crypto latency ~35s once; avg ~0.31s.
- **Crypto post-restart (~6300 ticks / ~18.0h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$246.7 / 84 fills / 8 open**. Shadows by PnL: **G≈+$246.7/84f > A≈F ≈+$149.5/93f > H ≈+$98.7/90f > D $0/0f > C≈−$196/57f > B≈−$210/69f > E≈−$927/1731f**. Regime mix all-run ~57% chop / ~43% trend (last-200 ~64% chop / 36% trend); trade_imbalance still ≈ 0. Live regime `trend` at reflect time.
- **G≠A:** 35/84 primary fills opposite side vs A (all chop fades; 35/35). **45 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A). **4 no-A**. `fade_applied=true` on **all 84/84** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs (`wrong_trend_opp=0`, `wrong_chop_same=0`).
- **Hourly story:** iter-28 G lead ~$87 (G≈+$229 vs A≈+$142) **holds and widens** — G now ≈+$247 vs A≈+$150 (G lead ~$97). Absolute G PnL rose (~+$17) while A edged up (~+$8). Still mostly MTM on existing shorts plus a few VVV/FARTCOIN/ZEN adds. **BONK** short still the main G driver: u≈+$191.7 on ~$1.41k notional. **FARTCOIN** short u≈+$85.1 on ~$1.32k. Other G open: API3/PYTH small green; VVV near flat; ZEC/XRP soft red; ZEN soft red (~−$23, reduced after a trend sell). No stale/zero-mid marks. A’s BONK remains small while G’s BONK short keeps the lead’s mechanical core.
- **Last ~1h:** **6** new G fills — VVV chop fade sells t5999/6002 (OPP vs A buys on `large_up`, correct fade); VVV trend buys t6004/6016 same-as-A; FARTCOIN trend sell t6059 no-A; ZEN trend sell t6103 same-as-A. Capacity still full at 8 names. Fill concentration all-run: BONK 31 / FARTCOIN 24 / VVV 16 / ZEN 6 / PYTH 3.
- **Shadows:** B/C stay deep red (no new thin-book spike contenders). E exit overlay ~21× G fills and deepest red (~−$927). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock RTH (~5.1h into open):** ~1062–1065 ticks; `session_ok` False×876 / True×188. Primary **A ≈ −$0.97 / 35 fills / 8 open** — shorts TSLA/GOOGL green; DIS/ARM soft red; META/SPY small; ZM/SNAP flat. Gave back prior ~+$3.5 → slight red (noise). Shadows: **G ≈+$4.5/34f ≳ B≈C ≈+$1.0/16–23f > D=0 > A≈F≈H ≈−$1.0/35f > E ≈−$2.9/99f**. Still noise-level; no stock primary call (`last_ts` ~11:33 PT).

### Lessons learned
1. **Good / KEEP validated afternoon RTH:** G lead held an eighth consecutive hour and **widened** (~$87→~$97) without a mid-wave CHANGE. Fade path still winning; two new VVV chop fades were correctly opposite A. Escalate rule (a) still not met — needs *sustained G≪A after RTH with fade losing on the same names across several post-open hours*.
2. **Watch / concentration:** BONK ~$1.4k short + FARTCOIN ~$1.3k short still dominate G path-dependence (together ~+$277 unrealized vs book +$247 — other names net soft, ZEN −$23). Lead remains mechanical/MTM; do not CHANGE on this alone.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (45 trend same-side cases). Side selection in trend still matches A when A also fills; chop fades still 100% opposite A. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$927). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns on crypto.
5. **Stock / still noise:** RTH gate working; A book full at 8 names but PnL oscillates ~+$3.5→−$1. Stock G’s tiny lead (~+$4.5) is not multi-hour durable. Need into close before any stock primary call.
...
### Next observe window
- Next hourly reflect ~12:08–12:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM holds through mid-afternoon RTH, whether the ~$97 lead stays after any A rebound, and whether stock A leaves noise territory into the afternoon.

## 2026-09-23 12:31 PT · iteration 30 · KEEP (G lead holds mid-afternoon · MTM giveback narrows lead · stock still noise)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~4.0h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=11` (was 7; +4 this window, non-fatal); stock `errors=1` (unchanged). Max crypto latency ~35s once; avg ~0.35s.
- **Crypto post-restart (~6570 ticks / ~18.9h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$200.0 / 86 fills / 8 open**. Shadows by PnL: **G≈+$200.0/86f > A≈F ≈+$122.6/94f > H ≈+$73.0/91f > D $0/0f > C≈−$185.7/57f > B≈−$208.1/70f > E≈−$975.8/1821f**. Regime mix all-run ~57% chop / ~43% trend (last-200 ~54% chop / 44% trend); trade_imbalance still ≈ 0. Live regime `chop`.
- **G≠A:** 36/86 primary fills opposite side vs A (all chop fades; 36/36). **45 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A). **5 no-A**. `fade_applied=true` on **all 86/86** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs (`wrong_trend_opp=0`, `wrong_chop_same=0`).
- **Hourly story:** iter-29 G lead ~$97 (G≈+$247 vs A≈+$150) **holds but narrows** — G now ≈+$200 vs A≈+$123 (G lead ~$77). Absolute G PnL gave back ~−$47 MTM while A also softened (~−$27); lead compression is shared mark-down on shorts, not a fade flip. **BONK** short still the main G driver: u≈+$163.1 on ~$1.44k notional (was ~+$192). **FARTCOIN** short u≈+$67.4 on ~$1.43k (was ~+$85; +1 trend sell this window). Other G open: API3/PYTH green; VVV near flat/−$3.5; ZEC/XRP soft red; ZEN soft red (~−$21, after a chop fade buy that reduced the long?). No stale/zero-mid marks. A’s BONK path still diverges; fade path-dependence remains the lead’s mechanical core.
- **Last ~1h:** **2** new G fills — FARTCOIN trend sell t6431 no-A (`large_down`, same-side momentum when A absent); ZEN chop fade buy t6466 OPP vs A sell on `large_down` (correct fade). Capacity still full at 8 names. Fill concentration all-run: BONK 31 / FARTCOIN 25 / VVV 16 / ZEN 7 / PYTH 3.
- **Shadows:** B/C stay deep red (no new thin-book spike contenders). E exit overlay ~21× G fills and deepest red (~−$976). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock RTH (~6.0h into open):** ~1104 ticks; `session_ok` False×877 / True×229. Primary **A ≈ −$2.1 / 42 fills / 8 open** — all shorts (GOOGL/TSLA/SNAP/META/SPY green-ish; DIS/ZM soft; ARM tiny weird avg_entry=0 noise). New this hour: GOOGL sells, ARM buys (cover/add), META sell, TSLA buy. Shadows: **G ≈+$2.7/39f > D=0 > A≈F≈H ≈−$2.1/42f ≳ C ≈−$2.7/17f > E ≈−$4.2/113f > B ≈−$4.8/28f**. Still noise-level; no stock primary call (`last_ts` ~12:29 PT).

### Lessons learned
1. **Good / KEEP validated mid-afternoon RTH:** G lead held a ninth consecutive hour (~$97→~$77) without a mid-wave CHANGE despite ~$47 absolute MTM giveback. Escalate rule (a) still not met — needs *sustained G≪A after RTH with fade losing on the same names across several post-open hours*. G remains #1 and both BONK/FARTCOIN shorts still green.
2. **Watch / concentration + MTM:** BONK ~$1.4k short + FARTCOIN ~$1.4k short still dominate (together ~+$230 unrealized vs book +$200 — other names net soft, ZEN −$21). Lead remains mechanical/MTM; giveback this hour was shared with A, not a fade bug. Do not CHANGE on this alone.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (45 trend same-side cases). Side selection in trend still matches A when A also fills; chop fades still 100% opposite A (incl. new ZEN fade). Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$976). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns on crypto. Crypto errors +4 this window (still ops noise, not strategy).
5. **Stock / still noise:** RTH gate working; A book full at 8 names but PnL oscillates ~+$3.5→−$1→−$2. Stock G’s tiny lead (~+$2.7) is not multi-hour durable. Need into close before any stock primary call.
...
### Next observe window
- Next hourly reflect ~13:08–13:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM stabilizes or keeps giving back through late afternoon RTH, whether the ~$77 lead stays above A after any rebound, and whether stock A leaves noise territory into the close.

## 2026-09-23 13:33 PT · iteration 31 · KEEP (G lead holds post-RTH close · MTM bounce · no new fills)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~3.0h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=22` (was 11; +11 this window, non-fatal ops noise); stock `errors=2` (was 1). Max crypto latency ~35s once; avg ~0.43s.
- **Crypto post-restart (~6882 ticks / ~19.9h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$226.2 / 86 fills / 8 open**. Shadows by PnL: **G≈+$226.2/86f > A≈F ≈+$149.1/94f > H ≈+$98.8/91f > D $0/0f > C≈−$176/57f > B≈−$220/72f > E≈−$1013/1901f**. Regime mix all-run ~57% chop / ~43% trend (last-200 ~54% chop / 46% trend); trade_imbalance still ≈ 0. Live regime `chop` at reflect time.
- **G≠A:** 36/86 primary fills opposite side vs A (all chop fades; 36/36). **45 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A). **5 no-A**. `fade_applied=true` on **all 86/86** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs (`wrong_trend_opp=0`, `wrong_chop_same=0`).
- **Hourly story:** iter-30 G lead ~$77 (G≈+$200 vs A≈+$123) **holds** — G now ≈+$226 vs A≈+$149 (G lead ~$77). Absolute G recovered ~+$26 MTM while A also recovered (~+$26); lead flat. **No new G fills this window** (last still t6466 ZEN chop fade ~12:08 PT); capacity full at 8 names — hour was pure MTM. **BONK** short still the main G driver: u≈+$179.5 on ~$1.42k notional (was ~+$163). **FARTCOIN** short u≈+$77.8 on ~$1.42k (was ~+$67). Other G open: API3/PYTH green; VVV flat; ZEC/XRP soft red; ZEN soft red (~−$23). No stale/zero-mid marks. Fade path-dependence (BONK/FARTCOIN shorts vs A’s divergent BONK path) remains the lead’s mechanical core.
- **Last ~1h:** **0** new G fills. Fill concentration all-run unchanged: BONK 31 / FARTCOIN 25 / VVV 16 / ZEN 7 / PYTH 3.
- **Shadows:** B/C stay deep red (no new thin-book spike contenders). E exit overlay ~22× G fills and deepest red (~−$1013). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock RTH closed (~13:00 PT):** ~1182–1186 ticks; `session_ok` False×911 / True×273. Primary **A ≈ +$2.91 / 42 fills / 8 open** — bounced from iter-30 ~−$2.1 on MTM (GOOGL/META/SNAP/TSLA/SPY green; ARM avg_entry=0 noise −$4.4; ZM/DIS soft). **0 new A fills** after t1083 (TSLA buy ~12:08 PT); book idle into/through the close. Shadows: **A≈F≈H ≈+$2.91 > G ≈+$1.81/39f > C ≈+$0.28/17f > D=0 > B ≈−$0.55/28f > E ≈−$4.1/132f**. Still noise-level; no stock primary call (`last_ts` ~13:30 PT, post-close).

### Lessons learned
1. **Good / KEEP validated post-RTH close:** G lead held a tenth consecutive hour (flat ~$77) without a mid-wave CHANGE. Absolute PnL bounced with A in lockstep — shared MTM, not a fade flip. Escalate rule (a) still not met — needs *sustained G≪A after RTH with fade losing on the same names across several post-open hours*. With RTH now closed, stock evidence window for this campaign is essentially done.
2. **Watch / concentration + idle book:** BONK ~$1.4k short + FARTCOIN ~$1.4k short still dominate (together ~+$257 unrealized vs book +$226 — other names net soft, ZEN −$23). Full 8-name capacity → zero new fills this hour. Lead remains mechanical/MTM; do not CHANGE on this alone.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (45 trend same-side cases). Side selection in trend still matches A when A also fills; chop fades still 100% opposite A. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$1013). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns on crypto. Crypto errors +11 this window (ops noise, not strategy).
5. **Stock / close = noise conclusion for this wave:** RTH gate worked; A finished the day ~flat (+$2.9 / 42f). No shadow edge survived the full RTH session. Keep stock primary **exp_A_short** through ends_ts; revisit only in a post-campaign wave if needed.

### Strategy decision · KEEP
- No strategy/code CHANGE. Keep crypto primary **exp_G_regime_mr**, stock primary **exp_A_short**, shadows A–H, same campaign id + ends_ts.
- Continue hourly RSI through the remaining ~3h window to FINAL; escalate only on (a) sustained G≪A with fade losing on the same names, (b) fade flag causing *wrong-side* trades, or (c) a clean multi-hour shadow edge (crypto-only now that RTH is closed).

### Code / config changes (if any)
- None this hour. (Optional later: fix `fade_applied` labeling on trend path so flag is false when regime=trend / no invert.)

### Next observe window
- Next hourly reflect ~14:08–14:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM holds into the final ~2–3h before ends_ts, whether the ~$77 lead stays above A, and prepare for FINAL at/after ends_ts (~16:30 PT) if both smokes have exited.

## 2026-09-23 14:30 PT · iteration 32 · KEEP (G lead narrows on MTM · still #1 · book idle)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~2.0h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=23` (was 22; +1 this window, non-fatal); stock `errors=3` (was 2). Max crypto latency ~35s once; avg ~0.55s.
- **Crypto post-restart (~7128 ticks / ~20.9h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$211.3 / 86 fills / 8 open**. Shadows by PnL: **G≈+$211.3/86f > A≈F ≈+$157.0/94f > H ≈+$107.0/91f > D $0/0f > C≈−$176/57f > B≈−$225/72f > E≈−$1007/1991f**. Regime mix all-run ~57% chop / ~43% trend (last-200 ~58% chop / 42% trend); trade_imbalance still ≈ 0. Live regime `trend` at reflect time.
- **G≠A:** 36/86 primary fills opposite side vs A (all chop fades; 36/36). **45 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A). **5 no-A**. `fade_applied=true` on **all 86/86** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs (`wrong_trend_opp=0`, `wrong_chop_same=0`).
- **Hourly story:** iter-31 G lead ~$77 (G≈+$226 vs A≈+$149) **holds but narrows** — G now ≈+$211 vs A≈+$157 (G lead ~$54). Absolute G gave back ~−$15 MTM while A rose ~+$8; lead compression is asymmetric MTM (BONK/FARTCOIN shorts softened; A’s larger FARTCOIN short + divergent book helped A), **not** a fade flip or wrong-side trade. **No new G fills this window** (last still t6466 ZEN chop fade ~12:08 PT); capacity full at 8 names — hour was pure MTM. **BONK** short still the main G driver: u≈+$171.3 on ~$1.43k notional (was ~+$179). **FARTCOIN** short u≈+$72.7 on ~$1.43k (was ~+$78). Other G open: API3/PYTH green; VVV flat; ZEC/XRP soft red; ZEN soft red (~−$26). No stale/zero-mid marks. Fade path-dependence remains the lead’s mechanical core.
- **Last ~1h:** **0** new G fills. Fill concentration all-run unchanged: BONK 31 / FARTCOIN 25 / VVV 16 / ZEN 7 / PYTH 3.
- **Shadows:** B/C stay deep red (no new thin-book spike contenders). E exit overlay ~23× G fills and deepest red (~−$1007). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock post-RTH close:** ~1242 ticks; `session_ok` False×1209 / True×377 (jsonl). Primary **A ≈ +$2.91 / 42 fills / 8 open** — unchanged vs iter-31 (idle MTM). Shadows: **A≈F≈H ≈+$2.91 > G ≈+$1.81/39f > C ≈+$0.28/17f > D=0 > B ≈−$0.55/28f > E ≈−$4.1/132f**. Noise conclusion for this wave stands; `last_ts` ~14:22 PT (idle).

### Lessons learned
1. **Good / KEEP validated late afternoon:** G lead held an eleventh consecutive hour (~$77→~$54) without a mid-wave CHANGE. G remains #1; both BONK/FARTCOIN shorts still green. Escalate rule (a) still not met — needs *sustained G≪A with fade losing on the same names across several hours*. Lead narrowed on MTM, not on fade bugs.
2. **Watch / lead compression + idle book:** BONK ~$1.4k short + FARTCOIN ~$1.4k short still dominate (together ~+$244 unrealized vs book +$211 — other names net soft, ZEN −$26). Full 8-name capacity → zero new fills for a second straight hour. A’s rebound capacity (larger FARTCOIN short) is real and trimmed the lead ~$23 this hour — watch into FINAL whether lead stays clearly positive.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (45 trend same-side cases). Side selection in trend still matches A when A also fills; chop fades still 100% opposite A. Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$1007). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns on crypto. Crypto errors +1 this window (ops noise).
5. **Stock / closed = noise for this wave:** Keep stock primary **exp_A_short** through ends_ts; no new evidence post-close.

### Strategy decision · KEEP
- No strategy/code CHANGE. Keep crypto primary **exp_G_regime_mr**, stock primary **exp_A_short**, shadows A–H, same campaign id + ends_ts.
- Continue hourly RSI through the remaining ~2h window to FINAL; escalate only on (a) sustained G≪A with fade losing on the same names, (b) fade flag causing *wrong-side* trades, or (c) a clean multi-hour shadow edge (crypto-only). At/after ends_ts (~16:30 PT), if both smokes have exited, write FINAL and retire the hourly iterate routine.

### Code / config changes (if any)
- None this hour. (Optional later: fix `fade_applied` labeling on trend path so flag is false when regime=trend / no invert.)

### Next observe window
- Next hourly reflect ~15:08–15:40 PT. Watch whether G’s BONK/FARTCOIN fade MTM holds into the final ~1–2h before ends_ts, whether the ~$54 lead stays above A, and prepare for FINAL at/after ends_ts (~16:30 PT) if both smokes have exited.

## 2026-09-23 15:26 PT · iteration 33 · KEEP (G still #1 into final hour · MTM giveback · 1 correct PYTH fade)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~1.1h left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=23` (unchanged); stock `errors=3` (unchanged). Max crypto latency ~35s once; avg ~0.54s.
- **Crypto post-restart (~7452 ticks / ~21.8h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$174.5 / 87 fills / 8 open**. Shadows by PnL: **G≈+$174.5/87f > A≈F ≈+$139.0/95f > H ≈+$89.4/92f > D $0/0f > C≈−$183/57f > B≈−$242/72f > E≈−$1018/2104f**. Regime mix all-run ~57% chop / ~43% trend (last-200 ~57% chop / 43% trend); trade_imbalance still ≈ 0. Live regime `trend` at reflect time.
- **G≠A:** 37/87 primary fills opposite side vs A (all chop fades; 37/37). **45 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A). **5 no-A**. `fade_applied=true` on **all 87/87** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs (`wrong_trend_opp=0`, `wrong_chop_same=0`).
- **Hourly story:** iter-32 G lead ~$54 (G≈+$211 vs A≈+$157) **holds but narrows** — G now ≈+$175 vs A≈+$139 (G lead ~$36). Absolute G gave back ~−$37 MTM while A softened ~−$18; lead compression is shared short mark-down plus A’s larger FARTCOIN short still outperforming G’s, **not** a fade flip. **1 new G fill** this window: **PYTH** chop fade sell t7206 (~14:42 PT) on `large_up`, OPP vs A buy — correct fade. Capacity still full at 8 names otherwise. **BONK** short still the main G driver: u≈+$157.0 on ~$1.44k notional (was ~+$171). **FARTCOIN** short u≈+$53.6 on ~$1.45k (was ~+$73). Other G open: API3/PYTH green; VVV flat; ZEC/XRP soft red; ZEN soft red (~−$25). No stale/zero-mid marks. Fade path-dependence remains the lead’s mechanical core.
- **Last ~1h:** **1** new G fill (PYTH). Fill concentration all-run: BONK 31 / FARTCOIN 25 / VVV 16 / ZEN 7 / PYTH 4 / API3 2.
- **Shadows:** B/C stay deep red (no new thin-book spike contenders). E exit overlay ~24× G fills and deepest red (~−$1018). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock post-RTH close:** ~1278 ticks; `session_ok` False×1005 / True×273. Primary **A ≈ +$2.91 / 42 fills / 8 open** — unchanged (idle MTM; last A fill still ~12:08 PT). Shadows: **A≈F≈H ≈+$2.91 > G ≈+$1.81/39f > C ≈+$0.28/17f > D=0 > B ≈−$0.55/28f > E ≈−$4.1/132f**. Noise conclusion for this wave stands; `last_ts` ~15:23 PT (idle).

### Lessons learned
1. **Good / KEEP validated into final hour:** G lead held a twelfth consecutive hour (~$54→~$36) without a mid-wave CHANGE. G remains #1; both BONK/FARTCOIN shorts still green. New PYTH chop fade was correctly opposite A. Escalate rule (a) still not met — needs *sustained G≪A with fade losing on the same names across several hours*.
2. **Watch / lead compression + concentration:** BONK ~$1.4k short + FARTCOIN ~$1.4k short still dominate (together ~+$211 unrealized vs book +$175 — other names net soft, ZEN −$25). Lead narrowed another ~$18 this hour on MTM; still clearly positive vs A into the last ~1h before ends_ts.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (45 trend same-side cases). Side selection in trend still matches A when A also fills; chop fades still 100% opposite A (incl. new PYTH). Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$1018). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns on crypto.
5. **Stock / closed = noise for this wave:** Keep stock primary **exp_A_short** through ends_ts; no new evidence post-close.

### Strategy decision · KEEP
- No strategy/code CHANGE. Keep crypto primary **exp_G_regime_mr**, stock primary **exp_A_short**, shadows A–H, same campaign id + ends_ts.
- Continue hourly RSI through the remaining ~1h window to FINAL; escalate only on (a) sustained G≪A with fade losing on the same names, (b) fade flag causing *wrong-side* trades, or (c) a clean multi-hour shadow edge (crypto-only). At/after ends_ts (~16:30 PT), if both smokes have exited, write FINAL and retire the hourly iterate routine.

### Code / config changes (if any)
- None this hour. (Optional later: fix `fade_applied` labeling on trend path so flag is false when regime=trend / no invert.)

### Next observe window
- Next hourly reflect ~16:08–16:40 PT (likely FINAL if ends_ts passed and smokes exited). Watch whether G’s BONK/FARTCOIN fade MTM holds the ~$36 lead into close of the wave.

## 2026-09-23 16:27 PT · iteration 34 · KEEP (G lead re-widens into final minutes · still #1 · 1 correct PYTH fade)

### Observed
- Campaign `G_primary_fade_fix_24h` still active; ends_ts `2026-09-23T23:30:55.713766+00:00` (~0.05h / ~3 min left). Crypto PID 1317447 + stock PID 1317448 + dashboard 1137915 **alive**. Crypto `errors=24` (was 23; +1 this window, non-fatal); stock `errors=3` (unchanged). Max crypto latency ~40s once; avg ~0.54s.
- **Crypto post-restart (~7800 ticks / ~22.8h from $10k re-init):** primary **`exp_G_regime_mr` ≈ +$189 / 88 fills / 8 open**. Shadows by PnL: **G≈+$189/88f > A≈F ≈+$135/96f > H ≈+$85/93f > D $0/0f > C≈−$186/57f > B≈−$249/73f > E≈−$1089/2196f**. Regime mix all-run ~57% chop / ~43% trend (last-200 ~84% chop / 16% trend); trade_imbalance still ≈ 0. Live regime `chop` at reflect time.
- **G≠A:** 38/88 primary fills opposite side vs A (all chop fades; 38/38). **45 same-side**, all `regime=trend` with `fade_applied=true` (momentum-correct vs A). **5 no-A**. `fade_applied=true` on **all 88/88** G fills (metadata always-on). Zero wrong-side trend or same-side chop bugs (`wrong_trend_opp=0`, `wrong_chop_same=0`).
- **Hourly story:** iter-33 G lead ~$36 (G≈+$175 vs A≈+$139) **re-widens** — G now ≈+$189 vs A≈+$135 (G lead ~$54). Absolute G recovered ~+$14 MTM while A softened ~−$4; lead expansion is shared short mark-up (BONK/FARTCOIN) plus A’s divergent book, **not** a fade flip. **1 new G fill** this window: **PYTH** chop fade buy t7655 (~16:00 PT) on `large_down`, OPP vs A sell — correct fade (covers toward flat on down-move fade). Capacity still full at 8 names otherwise. **BONK** short still the main G driver: u≈+$167.2 on ~$1.43k notional (was ~+$157). **FARTCOIN** short u≈+$58.4 on ~$1.44k (was ~+$54). Other G open: API3/PYTH green; VVV flat; ZEC/XRP soft red; ZEN soft red (~−$27). No stale/zero-mid marks. Fade path-dependence remains the lead’s mechanical core.
- **Last ~1h:** **1** new G fill (PYTH). Fill concentration all-run: BONK 31 / FARTCOIN 25 / VVV 16 / ZEN 7 / PYTH 5 / API3 2 / XRP 1 / ZEC 1.
- **Shadows:** B/C stay deep red (no new thin-book spike contenders). E exit overlay ~25× G fills and deepest red (~−$1089). H ≪ A ≡ F. D still 0 fills (imbalance≈0).
- **Stock post-RTH close:** ~1315 ticks; `session_ok` False×1042 / True×273. Primary **A ≈ +$2.91 / 42 fills / 8 open** — unchanged (idle MTM; last A fill still ~12:08 PT). Shadows: **A≈F≈H ≈+$2.91 > G ≈+$1.81/39f > C ≈+$0.28/17f > D=0 > B ≈−$0.55/28f > E ≈−$4.1/132f**. Noise conclusion for this wave stands; `last_ts` ~16:23 PT (idle).

### Lessons learned
1. **Good / KEEP validated into final minutes:** G lead held a thirteenth consecutive hour and **re-widened** (~$36→~$54) without a mid-wave CHANGE. G remains #1; both BONK/FARTCOIN shorts still green. New PYTH chop fade was correctly opposite A. Escalate rule (a) still not met — needs *sustained G≪A with fade losing on the same names across several hours*.
2. **Watch / concentration into FINAL:** BONK ~$1.4k short + FARTCOIN ~$1.4k short still dominate (together ~+$226 unrealized vs book +$189 — other names net soft, ZEN −$27). Lead recovered on MTM into the last ~3 min before ends_ts; still clearly positive vs A.
3. **Watch / fade flag:** Still `fade_applied=true` on every G fill including pure trend momentum (45 trend same-side cases). Side selection in trend still matches A when A also fills; chop fades still 100% opposite A (incl. new PYTH). Queue post-campaign label fix — **not** mid-wave strategy CHANGE.
4. **Bad / reinforced:** E exit overlay deepest red (~−$1089). H ≪ A ≡ F. D still dead (imbalance≈0). B/C remain failed spike patterns on crypto. Crypto errors +1 this window (ops noise).
5. **Stock / closed = noise for this wave:** Keep stock primary **exp_A_short** through ends_ts; no new evidence post-close.

### Strategy decision · KEEP
- No strategy/code CHANGE. Keep crypto primary **exp_G_regime_mr**, stock primary **exp_A_short**, shadows A–H, same campaign id + ends_ts.
- Smokes still running with ~3 min to ends_ts; **do not restart**. At/after ends_ts (~16:30 PT), if both smokes have exited, next reflect should write **FINAL** (likely FINAL·KEEP given G held #1 for the full post-restart window) and retire the hourly iterate routine. Escalate only on (a)/(b)/(c) — none met.
